...
//      m_bShowCubes = false;
  ...
  private:
    ...
    void processROSStereoImage(cv::Mat (&in)[LR], cv::Mat (&out)[LR])
    {
        // just resize
        for(int i=0;i<LR;i++){
#if 1 // no resize if almost the same size
            int col_diff = in[i].cols - out[i].cols;
            int row_diff = in[i].rows - out[i].rows;
            if (col_diff > 10 || col_diff < -10 ||
                row_diff > 10 || row_diff < -10 ) {
                cv::Mat tmp;
...
class VIVEnode
{
  public:
    ...
    rclcpp::Node::SharedPtr ros2_img_node;
    VRInterface vr_;
  ...
  , ros_clock(RCL_ROS_TIME)
{
...
#endif
    feedback_sub_ = ros2node->create_subscription<sensor_msgs::msg::JoyFeedback>("/vive/set_feedback", 2,
                                                                                 std::bind(&VIVEnode::set_feedback, this, _1));
#ifdef USE_IMAGE
  ros2_img_node = rclcpp::Node::make_shared("vive_image_node");
  image_transport::ImageTransport it(ros2_img_node);
  ...
  // compress version
#if 1
  sub_compress = ros2_img_node->create_subscription<sensor_msgs::msg::CompressedImage>("scene_camera/compressed_images", 30,
...
void VIVEnode::Run()
{
  ...
  std::thread th(std::bind(&VIVEnode::backGroundImage, this));
  ...
  while (rclcpp::ok())
  {
    //std::cerr << "ros cycle : " << run_hz_count << std::endl;
    // do stuff
    vr_.Update();
